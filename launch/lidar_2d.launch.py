...
def generate_launch_description():
    ...
    return LaunchDescription([
        Node(
            package='ffastllamaa', 
            executable='lidar_2d_conversion', 
            name='lidar_2d_conversion',
            remappings=[
                ('/input_pointcloud', '/car_1/lidar2d'),
                ('/converted_pointcloud', '/lidar_points')
            ],
            parameters=[
                {"ascending_time": False},
                {"distortion_free": False} # ONLY TRUE FOR SIMULATED DATA WITHOUT MOTION DISTORTION
            ],
            output='screen',
            on_exit=Shutdown()
        ),
